# weekly-planner[weekly-planner (8).html](https://github.com/user-attachments/files/26254380/weekly-planner.8.html)
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
.cell{height:36px;border-radius:5px;display:flex;align-items:center;justify-content:center;font-size:10px;text-align:center;line-height:1.2;padding:2px 3px;border:1px solid transparent;transition:filter .15s;letter-spacing:.02em;overflow:hidden;font-family:'DM Mono',monospace;}
...
<div class="act-panel">
  ...
  <div class="act-panel-hint">
    <b style="color:var(--text2);font-weight:500">Workout</b> — tap to cycle pilates / strength (+ 10m run) · max 4/week &nbsp;·&nbsp;
    <b style="color:var(--text2);font-weight:500">Hot yoga</b> — any day · max 2× · tap booked cell to remove &nbsp;·&nbsp;
    <b style="color:var(--text2);font-weight:500">Climbing</b> — any day; work days = gym next to work 5–7 PM &nbsp;·&nbsp;
    <b style="color:var(--text2);font-weight:500">Surf</b> — days off 3–4h block; work days = quick pre-work session
  </div>
...
<div class="forecast-bar">
  <h3>Surf forecast — playful 2–3 ft · occasional 4 ft · ≤3 kt wind · sunny</h3>
...
const YOGA_TIMES=['5 AM','6 AM','7 AM','8 AM','9 AM','10 AM','11 AM','12 PM','1 PM','2 PM','3 PM','4 PM','5 PM','6 PM','7 PM','8 PM','9 PM'];
...
function workSched(d){
  ...
  Object.assign(s,{'5 AM':{cls:'sleep',lb:'sleep'},'6 AM':{cls:'sleep',lb:'wake / prep'},'7 AM':{cls:'work s',lb:'shift start'},'8 AM':{cls:'work',lb:''},'9 AM':{cls:'work',lb:''},'10 AM':{cls:'work',lb:''},'11 AM':{cls:'work',lb:''},'12 PM':{cls:'work',lb:''},'1 PM':{cls:'work',lb:''},'2 PM':{cls:'work',lb:''},'3 PM':{cls:'work',lb:''},'4 PM':{cls:'work e',lb:'shift ends 5 PM'},'5 PM':{cls:'commute',lb:'commute ~40m'},'9 PM':{cls:'sleep',lb:''},'10 PM':{cls:'sleep',lb:''},'11 PM':{cls:'sleep',lb:'sleep'}});
  if(hasPreSurf){s['5 AM']={cls:'surf',lb:'surf'};s['6 AM']={cls:'surf',lb:'rinse + drive'};}
  appointments.filter(a=>a.day===d).forEach(a=>{s[timeToHour(a.time)]={cls:'appt',lb:a.desc||'appt'};});
  if(hasClimb){s['5 PM']={cls:'climb',lb:'climbing'};s['6 PM']={cls:'climb',lb:'(gym near work)'};s['7 PM']={cls:'free',lb:'commute home'};s['8 PM']={cls:'free',lb:'dinner / wind down'};}
  else if(hasYoga){const yt=(HI[yogaTime[d]]>=HI['6 PM'])?yogaTime[d]:'6 PM';s[yt]={cls:'yoga',lb:'hot yoga 1h'};const nx=HOURS[HI[yt]+1];if(nx)s[nx]={cls:'free',lb:'dinner'};s['8 PM']={cls:'free',lb:'wind down'};}
  else if(wt==='pilates'){s['6 PM']={cls:'pilates',lb:'pilates 45m'};s['7 PM']={cls:'pilates run',lb:'run 10m'};s['8 PM']={cls:'free',lb:'dinner / wind down'};}
  else if(wt==='strength'){s['6 PM']={cls:'strength',lb:'strength 40m'};s['7 PM']={cls:'strength run',lb:'run 10m'};s['8 PM']={cls:'free',lb:'dinner / wind down'};}
  else{s['6 PM']={cls:'free',lb:''};s['7 PM']={cls:'free',lb:'dinner'};s['8 PM']={cls:'free',lb:'wind down'};}
  return s;
}
...
function offSched(d){
  ...
  const s={};HOURS.forEach(h=>s[h]={cls:'free',lb:''});
  s['5 AM']={cls:'sleep',lb:'sleep'};s['9 PM']={cls:'sleep',lb:''};s['10 PM']={cls:'sleep',lb:''};s['11 PM']={cls:'sleep',lb:'sleep'};
  if(hasSurf){const ss=surfStart[d]||'7 AM';hoursFrom(ss,4).forEach((h,i,arr)=>{if(i===0)s[h]={cls:'surf',lb:'surf'};else if(i===arr.length-1)s[h]={cls:'surf',lb:'rinse + back'};else s[h]={cls:'surf',lb:''};});}
  if(hasYoga){const yt=yogaTime[d]||'9 AM';s[yt]={cls:'yoga',lb:'hot yoga 1h'};}
  if(hasClimb){const cs=climbTime[d]||'10 AM';s[cs]={cls:'climb',lb:'climbing'};const nx=HOURS[HI[cs]+1];if(nx)s[nx]={cls:'climb',lb:''};}
  if(ow==='pilates'){s['9 AM']={cls:'pilates',lb:'pilates 45m'};s['10 AM']={cls:'pilates run',lb:'run 10m'};}
  else if(ow==='strength'){s['9 AM']={cls:'strength',lb:'strength 40m'};s['10 AM']={cls:'strength run',lb:'run 10m'};}
  appointments.filter(a=>a.day===d).forEach(a=>{s[timeToHour(a.time)]={cls:'appt',lb:a.desc||'appt'};});
  return s;
}
...
function buildGrid(){
  ...
  HOURS.forEach(h=>{
    ...
    DAYS.forEach((_,i)=>{const s=scheds[i][h]||{cls:'free',lb:''};const cell=document.createElement('div');cell.className='cell '+s.cls;if(s.lb)cell.textContent=s.lb;grid.appendChild(cell);});
  });
}
...
function buildActPanel(){
  ...
  DAYS.forEach((_,i)=>{
    ...
    if(workDays[i]){
      ...
      if(wt==='pilates'){cell.className='wk-cell pilates-mode';cell.textContent='pilates';cell.title='tap → strength';}
      else if(wt==='strength'){cell.className='wk-cell strength-mode';cell.textContent='strength';cell.title='tap → off';}
      else{const atCap=countWk()>=4;cell.className='wk-cell inactive';cell.style.cursor=atCap?'not-allowed':'pointer';cell.style.opacity='0.5';cell.textContent=atCap?'max 4':'off';cell.title=atCap?'4 workout days reached':'tap → pilates';}
      ...
      if(ow==='pilates'){cell.className='wk-cell pilates-mode';cell.textContent='pilates';cell.title='makeup · tap → strength';}
      else if(ow==='strength'){cell.className='wk-cell strength-mode';cell.textContent='strength';cell.title='makeup · tap → off';}
      else{const atCap=countWk()>=4;cell.className='wk-cell inactive';cell.style.cursor=atCap?'not-allowed':'pointer';cell.style.opacity='0.4';cell.textContent=atCap?'max 4':'+ add';cell.title=atCap?'4 workout days reached':'add makeup workout';}
  ...
  DAYS.forEach((_,i)=>{
    ...
    if(isOn){
      const lbl=document.createElement('div');lbl.className='yc-label';lbl.textContent='booked ✕';
  ...
  DAYS.forEach((_,i)=>{
    ...
    const cell=document.createElement('div');
    if(!isOn&&count>=2){cell.className='opt-cell opt-locked';cell.textContent='max 2×';}
    else if(isOn){
      ...
      const lbl=document.createElement('div');lbl.style.cssText='font-size:10px;line-height:1;';lbl.textContent=workDays[i]?'post-work ✕':'booked ✕';
      cell.addEventListener('click',()=>{climbDays[i]=false;save();render();});
      if(workDays[i]){const note=document.createElement('div');note.style.cssText='font-size:9px;opacity:.6;';note.textContent='5–7 PM · gym near work';cell.appendChild(lbl);cell.appendChild(note);}
      else{
        const sel=document.createElement('select');
      ...
      cell.className='opt-cell';cell.textContent='+ add';
      cell.title=workDays[i]?'Gym next to work · 5–7 PM':'2h session';
  ...
  DAYS.forEach((_,i)=>{
    ...
    const cell=document.createElement('div');
    if(isWork){
      if(!surfable){cell.className='surf-cell surf-locked';cell.textContent='no go';}
      else if(!isWorkSurf&&totalSurf>=2){cell.className='surf-cell surf-locked';cell.textContent='max 2×';}
      else if(isWorkSurf){
        cell.className='surf-cell surf-on';
        const lbl=document.createElement('div');lbl.className='sc-label';lbl.textContent='pre-work 🤙 ✕';
        const note=document.createElement('div');note.style.cssText='font-size:9px;opacity:.6;';note.textContent='5–6 AM · quick session';
        cell.style.cursor='pointer';cell.addEventListener('click',()=>{surfWorkDay[i]=false;save();render();});
        cell.appendChild(lbl);cell.appendChild(note);
      }else{cell.className='surf-cell';cell.textContent='+ pre-work';cell.title='Quick 1h before 7 AM shift';cell.addEventListener('click',()=>{surfWorkDay[i]=true;save();render();});}
    }else if(!surfable){cell.className='surf-cell surf-locked';cell.textContent='no go';}
    else if(!isOn&&totalSurf>=2){cell.className='surf-cell surf-locked';cell.textContent='max 2×';}
    else if(isOn){
      cell.className='surf-cell surf-on';
      const lbl=document.createElement('div');lbl.className='sc-label';lbl.textContent='go 🤙 ✕';
      const sel=document.createElement('select');
      ...
      cell.appendChild(lbl);cell.appendChild(sel);
    }else{cell.className='surf-cell';cell.textContent='+ add';cell.addEventListener('click',()=>{surfDays[i]=true;save();render();});}
    grid.appendChild(cell);
  });
}
...
function buildForecast(){
  ...
  const INFO={good:{wave:'🌊',size:'3–4 ft',tag:'pumping · ≤3kt',cls:'surfable'},ok:{wave:'〰',size:'2–3 ft',tag:'playful · sunny',cls:'surfable ok-surf'},poor:{wave:'—',size:'flat / blown',tag:'no go',cls:'flat'}};
  DAYS.forEach((day,i)=>{
    const cond=surfFcast[i],info=INFO[cond];
    const card=document.createElement('div');card.className='fday '+info.cls;
    card.innerHTML=`<div class="fd-name">${day}</div><div class="fd-wave">${info.wave}</div><div class="fd-size">${info.size}</div><div class="fd-tag">${info.tag}</div><select><option value="good" ${cond==='good'?'selected':''}>pumping</option><option value="ok" ${cond==='ok'?'selected':''}>playful</option><option value="poor" ${cond==='poor'?'selected':''}>no go</option></select>`;
...
function buildStats(){
  ...
  DAYS.forEach((_,i)=>{if(workDays[i]){if(yogaDays[i]||climbDays[i])return;if(workoutType[i]==='pilates')pilates++;if(workoutType[i]==='strength')strength++;}else{if(offWorkout[i]==='pilates')pilates++;if(offWorkout[i]==='strength')strength++;}});
...
if('serviceWorker' in navigator){
  const swCode=`
    ...
    self.addEventListener('fetch',e=>{e.respondWith(caches.match(e.request).then(r=>r||fetch(e.request).catch(()=>new Response('',{status:408}))));});
  `;
